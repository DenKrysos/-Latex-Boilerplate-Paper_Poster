...
This one here is quite sophisticated. A simplified one can be found:
...
Just saying, there is also another Boilerplate for Book-type Documents:
...
There are conferences/journals/... for which I have proper layouts prepared and others for which I haven't. Sometimes they only want a compiled .pdf; sometimes they want the editable source submitted; sometimes the source-project shall even follow a certain file-structure, which then mostly means having everything inside one File.

Nonetheless, I am doing it the following way: I do always write manuscripts using this, my sophisticated Boilerplate. Because then, I am flexible in doing everything. Here again it comes in handy that with this Boilerplate, the content and all the LaTeX Overhead are well separated.<br />
So when finished with the manuscript, I can easily compile a preprint and a version for submission. Even when the submission requires a certain file-structure, then I work with "Sub-Directories" within the one projects (which so-to-speak represent own distinct "Sub-Projects").
I create a Directory on the Top-Level called "DenKrScaffold" and put everything in there. Just directly move it. Inside 2ProjectSetup.tex the Paths are then prepended with this DenKrScaffold/ and one time inside the 1main.tex to include the 2ProjectSetup.<br />
Then in parallel on the Top-Level, I set-up a set of files for the submission guidelines and copy the content over (which is quick and easy, because it is in this separate files in the "4chapter"-Directory).
So I create a new File, call it 1_main.tex and put the corresponding template for the Journal/Magazine/Conference to submit to in it. Directly in that file, I add my required additional preamble stuff and then into the \\begin/end{Document} area, I copy the content, well visible marked with a big comment. Also alongside created is a bibliography file 2_literature.bib as a copy an finally objects to include, like figure (which are prepended with '4\_'.

That might be not the most elegant solution, conceivable in principle, because then you have two distinct copies of your content, but in such a case that cannot be avoided anyways. Because a preprint has to be done and this cannot be performed within the submission-set-up. Hence, one may as well directly start with the "preprint-Version", which can be done through my Boilerplate and then only for final submission, the two Project-Sub-Setups diverge.<br />
When then coming back for revising the manuscript after review, I do prepare the follow-up version in either the preprint- or submission-document and then update the respectively other.<br />
That proceeding works quite quell and helps keeping things in order and everything condensed on one place.

## Project Divergence

A lot of Files in here are linked from other Projects.<br />
When working "Git-based", this won't be an issue anytime, because then already distinct standalone copies of these files are created.
...
Actually, for most cases, I would recommend performing a "Deep Copy" of the Project. I.e. not taking over the links referring to files in other projects, but having own duplicates so that the new Project works fully standalone.
...
The ReadOnly-Link to the Overleaf-Project for further Use:
